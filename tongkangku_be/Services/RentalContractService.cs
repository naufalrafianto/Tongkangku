...
namespace tongkangku_be.Services
{
        ...
        ApplicationDbContext context) : IRentalContractService
    {
        ...
        private readonly ILaytimeRecordRepository _laytimeRecordRepository =
        ...
        public async Task<RentalContractResponseDto> GetByIdAsync(Guid id)
        {
            ...
            return contract == null
                ? throw new NotFoundException(
                    $"Rental contract with id '{id}' was not found.")
        ...
        public async Task<List<RentalContractResponseDto>> GetAllAsync()
        {
            ...
            return contracts
                .Select(RentalContractMapper.ToDto)
                .ToList();
        }

        public async Task<RentalContractResponseDto> GetByRentalRequestIdAsync(
        ...
        public async Task<RentalContractStatusResponseDto> CreateAsync(
            CreateRentalContractDto dto)
        {
            ...
            {
                throw new NotFoundException(
                    $"Rental offer with id '{dto.OfferId}' was not found.");
            }
            ...
            if (offer.Status != RentalOfferStatus.Accepted)
            {
                ...
                {
                    OfferId =
                        "A contract can only be generated from an accepted offer."
                });
            }

            return await CreateContractFromOfferAsync(offer);
        }

        public async Task<RentalContractStatusResponseDto> CreateFromAcceptedOfferAsync(
            Guid offerId)
        {
            var offer = await _rentalOfferRepository.GetByIdAsync(offerId);

            if (offer == null)
            {
                throw new NotFoundException(
                    $"Rental offer with id '{offerId}' was not found.");
            }
        ...
        private async Task<RentalContractStatusResponseDto> CreateContractFromOfferAsync(
            RentalOffer offer)
        {
            var alreadyExists =
                await _rentalContractRepository
                    .ExistsForRentalRequestAsync(
            ...
            if (rentalRequest.Cargos == null ||
                rentalRequest.Cargos.Count == 0)
            {
                throw new ValidationException(new
                {
                    RentalRequestId =
                        "Cannot generate a contract without cargo details."
                });
            }

            var vessel = await _vesselRepository.GetByIdAsync(
            ...
            return await _context.ExecuteInTransactionAsync(async () =>
            {
                ...
                rentalRequest.Status = RentalRequestStatus.Contracted;
                rentalRequest.UpdateAt = DateTime.UtcNow;
        ...
        public async Task<RentalContractStatusResponseDto> CompleteAsync(
            Guid id)
        {
            var contract =
                await _rentalContractRepository.GetByIdAsync(id);
            ...
            var laytimeRecords =
                await _laytimeRecordRepository
                    .GetByContractIdAsync(contract.Id);

            var totalLaytimeAdjustment =
                laytimeRecords.Sum(x => x.NetLaytimeAmount);

            var finalSettlement =
                (contract.AgreedTotalPrice ?? 0) +
                totalLaytimeAdjustment;

            return await _context.ExecuteInTransactionAsync(async () =>
            {
                contract.Status =
                    RentalContractStatus.Complete;

                contract.TotalLaytimeAdjustment =
                    totalLaytimeAdjustment;

                contract.FinalSettlementAmount =
                    finalSettlement;
        ...
        public async Task<RentalContractStatusResponseDto> CancelAsync(
            Guid id)
        {
            var contract =
                await _rentalContractRepository.GetByIdAsync(id);
        ...
        private async Task<string> GenerateContractNumAsync(
            DateTime startDate)
        {
